...
Page({
  ...
  querySurveyCount(){
    ...
    survey.equalTo('owner', app.globalData.user.objectId);
  ...
  querySurveyList(){  
    ...
    let skip = page*limit;
    const cql = `select title,voteNums  from survey where owner = '${app.globalData.user.objectId}' limit ${skip}  order by createdAt desc`; 
  ...
  edit(e){
    ...
    wx.navigateTo({
      url: `/pages/vote/vote_edit?surveyId=${surveyId}`
    })
