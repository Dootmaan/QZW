...
Page({ 
  ...
  fetchPublishs: function () {
    ...
    const query = new AV.Query(Publish).descending('createdAt');
  ...
  //   const setPublishs = this.setPublishs.bind(this);
  //   return AV.Promise.all([query.find().then(setPublishs)]).catch(error => console.error(error.message));
  //   // console.log(this.data.publishs);
  // },  
  onReady: function () { 
  ...
  setPublishs: function (publishs) {
    ...
    //   console.log(this.getObjectTime(publishs[i]))
    // }
    return publishs;
  },
  getUserInfo(cb) {
  ...
  comment(e){
    ...
    var userAvatarUrl = e.currentTarget.dataset.useravatarurl;
    var pubtime = e.currentTarget.dataset.pubtime;
    //这些data属性自动会被转化成小写
    // console.log("邻里圈id"+publish);
    // console.log(userNickName);
    // console.log(userAvatarUrl);
    wx.navigateTo({
      url: '/pages/publish/publish_hall_comment?publish=' + publish + '&userNickName=' + userNickName + '&userAvatarUrl=' + userAvatarUrl + '&pubtime=' + pubtime,
    })
  },
  likePublish(e){
    console.log(e);
  ...
  shieldPublish(e){
    ...
    new AV.Query('Publish').get(publishId).then(
